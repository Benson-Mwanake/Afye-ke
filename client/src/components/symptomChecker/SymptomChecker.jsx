...
Light UI that calls OpenAI service (if configured).
  ...
  return (
    <div className="bg-white p-6 rounded shadow">
      ...
      <form onSubmit={submit} className="space-y-3">
        <textarea value={input} onChange={e=>setInput(e.target.value)} placeholder="Describe your symptoms..." className="w-full p-2 border rounded" />
        <div className="flex items-center gap-3">
          <button className="bg-afyaBlue text-white px-4 py-2 rounded">{loading ? 'Checking...' : 'Check'}</button>
        </div>
      </form>
